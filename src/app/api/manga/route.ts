...
export async function GET(req: NextRequest) {
  try {
    ...
    // Query all manga
    const totalCount = await mangaCol
      .countDocuments();
    const manga = await mangaCol
      .find({})
      .sort(sortObj)
      .skip(skip)
      .limit(limit)
...
// POST: Upload new manga or chapter
export async function POST(req: NextRequest) {
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      // Save cover image file if present (manga cover)
    ...
    if (chapterUpload === '1') {
      ...
      let coverPageUrl: string | undefined;
      let pdfUrl: string | undefined;

      if (!isJson) {
        // Only form-data supports files
        const formData = await req.formData();
        const coverFile = formData.get('coverPage') as any;
        const pdfFile = formData.get('pdfFile') as any;
        const storage = getStorage();

        if (coverFile && typeof coverFile === 'object' && 'arrayBuffer' in coverFile) {
          const buf = Buffer.from(await coverFile.arrayBuffer());
          const key = `chapter_cover_${mangaId}_${chapterNumber}_${Date.now()}_${coverFile.name}`;
          coverPageUrl = await storage.save({ buffer: buf, key, contentType: coverFile.type || 'image/webp' });
        }
        if (pdfFile && typeof pdfFile === 'object' && 'arrayBuffer' in pdfFile) {
          const buf = Buffer.from(await pdfFile.arrayBuffer());
          const key = `chapter_pdf_${mangaId}_${chapterNumber}_${Date.now()}_${pdfFile.name}`;
          pdfUrl = await storage.save({ buffer: buf, key, contentType: 'application/pdf' });
        }
      }

      const chaptersCol = db.collection('chapters');
      const newChapter: any = {
        mangaId: typeof mangaId === 'string' ? mangaId : String(mangaId),
        chapterNumber,
        subtitle: subtitle || '',
        description,
      ...
      const insert = await chaptersCol.insertOne(newChapter);
      return NextResponse.json({ success: true, message: 'Chapter uploaded successfully!', chapter: { ...newChapter, _id: insert.insertedId.toString(), createdAt: newChapter.createdAt.toISOString() } });
    }
