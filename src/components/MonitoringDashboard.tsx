...
import { AlertTriangle, Activity, RefreshCw, Eye, EyeOff, BarChart3, AlertCircle, CheckCircle, XCircle } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
          ...
          <div className="flex items-center gap-4 mt-0">
            <button
              ...
              disabled={isLoading}
              className="focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 transition-transform duration-150 hover:scale-105 focus:scale-105 flex items-center gap-2 px-4 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white rounded-lg"
            >
            ...
            <button
              ...
              className={`focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 transition-transform duration-150 hover:scale-105 focus:scale-105 flex items-center gap-2 px-4 rounded-lg ${
                autoRefresh ? 'bg-green-600 hover:bg-green-700 text-white' : 'bg-gray-600 hover:bg-gray-700 text-white'
              }`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-lg shadow p-6">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-2">{errorStats?.today.total || 0}</h3>
            <p className="text-gray-600">Errors</p>
            {errorStats?.today.total ? (
              <div className="mt-2 text-sm text-red-600">{errorStats.today.severities?.critical || 0} critical</div>
            ) : null}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white rounded-lg shadow p-6">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-2">{performanceStats?.today.totalSessions || 0}</h3>
            <p className="text-gray-600">Sessions</p>
            {performanceStats?.today.totalSessions ? (
              <div className="mt-2 text-sm text-blue-600">
                Avg: {(() => {
                  const scores = performanceStats?.today.scores || [];
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white rounded-lg shadow p-6">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-2">{errorStats?.weekly.total || 0}</h3>
            <p className="text-gray-600">Total Errors</p>
            {errorStats?.weekly.critical ? (
              <div className="mt-2 text-sm text-orange-600">{errorStats.weekly.critical} critical</div>
            ) : null}
          </motion.div>

          {/* Weekly Performance */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between mb-4">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-2">{performanceStats?.weekly.averageScore || 0}</h3>
            <p className="text-gray-600">Average Score</p>
            {performanceStats?.weekly.totalSessions ? (
              <div className="mt-2 text-sm text-green-600">{performanceStats.weekly.totalSessions} sessions</div>
            ) : null}
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Recent Errors */}
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} className="bg-white rounded-lg shadow">
                  ...
                  {recentErrors.map((error, index) => (
                    <div key={index} className="border-l-4 border-red-500 pl-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`px-2 py-1 rounded text-xs font-medium ${error.severity === 'critical' ? 'bg-red-100 text-red-800' : error.severity === 'high' ? 'bg-orange-100 text-orange-800' : error.severity === 'medium' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'}`}>
                          {error.severity}
                        </span>
                      </div>
          ...
          {/* Recent Performance */}
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }} className="bg-white rounded-lg shadow">
            <div className="p-6 border-b border-gray-200">
                  ...
                  {recentPerformance.map((perf, index) => (
                    <div key={index} className="border-l-4 border-blue-500 pl-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`px-2 py-1 rounded text-xs font-medium ${getPerformanceGradeColor(perf.performanceGrade)}`}>{perf.performanceGrade}</span>
                      </div>
                      <div className="text-sm mb-1">Score: {perf.performanceScore}/100</div>
        ...
        {performanceStats?.weekly.gradeCounts && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="mt-8 bg-white rounded-lg shadow p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {Object.entries(performanceStats.weekly.gradeCounts).map(([grade, count]) => (
                <div key={grade} className="text-center">
                  <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-2 text-2xl font-bold ${getPerformanceGradeColor(grade)}`}>{grade}</div>
